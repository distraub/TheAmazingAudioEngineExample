...
#import "ABRemoteButtonTrigger.h"
#import "ABRemoteVariableTrigger.h"

#define ABSDKVersionString @"2.0b20"
...
@section Dont-Panic Don't Panic!
 ...
 - then [creating instances](@ref Create-Properties) of the Audiobus Controller, 
   [receiver](@ref Instantiate-Receiver-Port) and/or [sender](@ref Instantiate-Sender-Port)
 ...
 [Audiobus sender and/or receiver ports](@ref Sending-Receiving) directly, after
 ...
 or building [Audiobus filters](@ref Filtering), then this document
...
@section Capabilities Capabilities: Inputs, Outputs and Filters
 ...
 Receiving and Filtering. Your app can perform several of these roles at once. You create sender,
...
@page Integration-Guide Integration Guide
 ...
 will still be relevant, but you'll need to read on to learn about [Sending](@ref Audiobus-Sender)
 and [Receiving](@ref Audiobus-Receiver) using the API directly. Don't worry - this is still
 ...
 Audio Unit Wrapper, then you can interact with the receiver and sender ports directly.
 These easy-to-use classes provide C functions to transmit and receive audio. See
 [Common Recipes](@ref Recipes), [Sending](@ref Audiobus-Sender) and [Receiving](@ref Audiobus-Receiver)
 for more info.
...
@section General-Principles General Design Principles
 ...
 Audiobus' sender port is extremely light when not connected: The send function ABSenderPortSendAudio 
...
@section Instantiate-Wrapper 10. Create Ports and Instantiate and Configure the Audiobus Audio Unit Wrapper
 ...
 @subsection Instantiate-Sender-Port Sender Port
 
 If you intend to send audio, then you'll need to create an sender port.
 ...
 @code
 ABSenderPort *sender = [self.audiobusController addSenderPortNamed:@"Audio Output"
                                                              title:NSLocalizedString(@"Main App Output", @"")];
 @endcode
 
 > If you are using an sender port and *not* using the Audio Unit Wrapper, you **must** handle system audio
 > output appropriately, muting or playing your app's audio output depending on the presence of the
 > @link ABReceiverPortAttributePlaysLiveAudio @endlink flag on the receiver port's
 > @link ABReceiverPort::attributes attributes @endlink property. See [Designated Output](@ref Designated-Output)
 > for details.

 @subsection Instantiate-Receiver-Port Receiver Port
 
 Similarly, if you intend to receive audio, then create an receiver port:

 @code
 ABReceiverPort *receiver = [self.audiobusController addReceiverPortNamed:@"Audio Input"
                                                                    title:NSLocalizedString(@"Main App Input", @"")];
 @endcode
 
 Next, you may need to configure the receiver port to work with your app.
 ...
 <img src="plays-live-audio.png" title="Demonstration of the ABReceiverPortAttributePlaysLiveAudio flag" />
 
 You can tell Audiobus that your app produces audio output based on the audio received from the receiver port by setting
 the @link ABReceiverPortAttributePlaysLiveAudio @endlink flag on the receiver port's
 @link ABReceiverPort::attributes attributes @endlink property:
 
 @code
 receiver.attributes = ABReceiverPortAttributePlaysLiveAudio;
 @endcode
 
 This will cause sources connected to your receiver port to mute their output to avoid duplicate audio
 ...
 @code
 receiver.icon = [UIImage imageNamed:@"Port-Icon.png"];
 @endcode
 
 If you wish to receive multi-channel audio, with one audio stream for each connected app, then you'll need
 to interact with the [receiver port directly](@ref Receive-Streams). If this is the case, you can simply
 pass `nil` to the Audio Unit Wrapper for the input parameter.

 @subsection Audio-Unit-Wrapper Audio Unit Wrapper
 
 Now create the Audio Unit Wrapper, passing in the primary sender and/or receiver ports (pass `nil` for any
 you don't need).
 ...
 property, or via [ABReceiverPortReceiveLive](@ref ABReceiverPort::ABReceiverPortReceiveLive)) for *recording*.
 ...
 **If, however, you are receiving audio from a render callback, there are two very important things you
 ...
 a call to [ABReceiverPortReceive](@ref ABReceiverPort::ABReceiverPortReceive) instead, placed within a loop
 ...
 while ( hasMoreAudio ) {
     ...
     if ( ABReceiverPortIsConnected(THIS->_receiverPort) ) {
         // Our receiver port is connected
         int bufferSizeToProcess = inNumberFrames;
         
         frames = ABReceiverPortPeek(THIS->_receiverPort, &timestamp);
         ...
         ABReceiverPortReceive(THIS->_receiverPort, nil, bufferList, &frames, NULL, NULL);
 ...
 [ABReceiverPortPeek](@ref ABReceiverPort::ABReceiverPortPeek) (note that this is already done in the sample 
 code above):
 
 @code
 AudioTimeStamp timestamp;
 ABReceiverPortPeek(THIS->_receiverPort, &timestamp);
 @endcode
...
@section Test 11. Test
 ...
 If you're creating an app with receiver ports, then try the "AB Sender" and "AB Torture Test" samples.
 
 If you're creating an app with sender ports, then try the "AB Receiver" sample, which both records audio and
...
@section Youre-Done You're Done!
 ...
 [Audiobus Receiver Port](@ref Audiobus-Receiver), so you use the correct, low-latency audio stream.
 ...
 > Please note that if you set `allowsMultipleInstancesInConnectionGraph` to YES and you have both receiver and sender ports,
 > [ABReceiverPortReceiveLive](@ref ABReceiverPort::ABReceiverPortReceiveLive) will return silence if your receiver port is connected to
 > your sender port, to avoid feedback issues.
 ...
 [receiving](@ref Audiobus-Receiver), exposing multiple audio [ports](@ref Ports),
 or building [filter apps](@ref Filtering)
...
@page Recipes Common Recipes
 ...
 @section Audio-Unit-Wrapper-Recipe Create receiver and sender ports and use the Audio Unit Wrapper

 This sample describes the basic scenario using one receiver port and one sender port with the
 ...
 self.audiobusAudioUnitWrapper = [[[ABAudiobusAudioUnitWrapper alloc]
                                     initWithAudiobusController:self.audiobusController
                                                      audioUnit:self.audioController.audioUnit
                                                         output:[self.audiobusController addSenderPortNamed:@"Main" title:@"Main Output"]
                                                          input:[self.audiobusController addReceiverPortNamed:@"Main" title:@"Main Input"]]
                                        autorelease];
 @endcode

 @section Input-Port-Recipe Create an receiver port and manually receive audio
 ...
 called whenever audio arrives at the receiver port.
 
 @code
 ABReceiverPort *receiver = [self.audiobusController addReceiverPortNamed:@"Main" title:NSLocalizedString(@"Main Input", @"")];
 receiver.clientFormat = myAudioDescription;
 ...
 receiver.audioInputBlock = ^(ABReceiverPort    *receiverPort,
                              UInt32          lengthInFrames,
                              AudioTimeStamp *nextTimestamp,
                              ABPort         *sourcePortOrNil) {

     // Receive audio into buffer list
     ABReceiverPortReceive(receiverPort, sourcePortOrNil, bufferList, &lengthInFrames, nextTimestamp, NULL);
 ...
 @section Sender-Port-Recipe Create an sender port and send audio

 This code snippet demonstrates how to create an sender port, and then send audio through it.
 
 It also demonstrates how to monitor the @link ABReceiverPortAttributePlaysLiveAudio @endlink
 attribute and mute the audio output as necessary - see [Designated Output](@ref Designated-Output) for
 details.
 
 @code
 // In app initialisation...
 self.sender = [self.audiobusController addSenderPortNamed:@"Main" title:@"Main Output"];
 self.sender.clientFormat = myAudioDescription;
 
 // In an input callback/etc
 ABSenderPortSendAudio(_sender, ioData, inNumberFrames, inTimestamp, NULL);
 
 if ( ABSenderPortGetConnectedPortAttributes(_sender) & ABReceiverPortAttributePlaysLiveAudio ) {
 ...
 - (void)connectionsChanged:(NSNotification*)notification {
    if ( _audiobusController.connected ) {
        ...
        // Note: You can also use the functions ABReceiverPortIsConnected, ABSenderPortIsConnected
        // and ABFilterPortIsConnected to determine which specific receiver/sender/filter ports
        // are connected.
 ...
 @section Enumerate-Connections Enumerate apps connected to an receiver port
 
 This illustrates how to inspect each individual source of an receiver port. This information
 ...
 `ABPortTypeFilter`, and a list of all destinations with the `ABPortTypeReceiver`.
 ...
 - (void)audiobusConnectionsChanged:(NSNotification*)notification {
     ...
     for ( int i=0; i<kMaxSources; i++ ) {
         if ( _portTable[i].port && ![_receiver.sources containsObject:_portTable[i].port] ) {
 ...
 static void MyAQInputCallback(void *inUserData,
    ...
    ABReceiverPortReceiveAQ(THIS->_audiobusReceiverPort,
                         nil,
                         inBuffer,
                         &inNumPackets,
                         &timestamp,
 ...
 @section Receiver-Port-Multichannel Receive multi-channel audio
 
 This sample code describes a scenario where the app has a single receiver port that receives separate
 audio streams from each source, and records them to a fictional track model object.
 
 The app creates an receiver port and directs it to receive audio as separate streams. Then it begins
 observing connection change notifications.
 
 When the connection changes, the sample code enumerates added and removed sources of the receiver port,
 ...
 whenever audio arrives at the app's receiver port.
 ...
 [ABReceiverPortReceive](@ref ABReceiverPort::ABReceiverPortReceive)/[ABReceiverPortPeek](@ref ABReceiverPort::ABReceiverPortPeek)
 without an [audioInputBlock](@ref ABReceiverPort::audioInputBlock), to allow you to do things like receive
 audio from within the Core Audio thread, such as from inside an input callback. To do so, however,
 certain care must be taken. Please see the documentation for [ABReceiverPortReceive](@ref ABReceiverPort::ABReceiverPortReceive)
 and [ABReceiverPortEndReceiveTimeInterval](@ref ABReceiverPort::ABReceiverPortEndReceiveTimeInterval) for more info.
 ...
 @code
 self.receiverPort = [self.audiobusController addReceiverPortNamed:@"Main" title:NSLocalizedString(@"Main Input", @"")];
 _receiverPort.clientFormat = myAudioDescription;
 _receiverPort.receiveMixedAudio = NO;
 ...
 _receiverPort.audioInputBlock = ^(ABReceiverPort    *receiverPort,
                                UInt32          lengthInFrames,
                                AudioTimeStamp *nextTimestamp,
                                ABPort         *sourcePort) {
 
     // Receive audio for source sourcePort into buffer list
     ABReceiverPortReceive(receiverPort, sourcePort, bufferList, &lengthInFrames, nextTimestamp, NULL);
 ...
 - (void)connectionsChanged:(NSNotification*)notification {
    for ( ABPort *port in _receiverPort.sources ) {
    ...
    for ( NSValue *portKey in [_sourceToTrackMapping allKeys] ) {
        ...
        if ( ![_receiverPort.sources containsObject:port] ) {
...
@page Ports Ports
 ...
 Audiobus ports are the endpoints for audio. [Receiver ports](@ref Audiobus-Receiver) receive audio,
 [sender ports](@ref Audiobus-Sender) send it, and [filter ports](@ref Filtering) process received
 audio and send it onwards.
 
 Advanced apps can produce more than one separate audio stream by creating multiple sender ports,
 or accept audio input to different places by creating multiple receiver ports.
 ...
 @link ABAudiobusController::addReceiverPortNamed:title: addReceiverPortNamed:title: @endlink and
 @link ABAudiobusController::addFilterPortNamed:title:processBlock: addFilterPortNamed:title:processBlock: @endlink
 methods.
 
 For example, a multi-track player might define one sender port for every track, in addition to
 the main port. Then, certain tracks could be routed to other iOS devices, or filtered by a
 filter app.
 
 Alternately, a mixer app might define several receiver ports that apps can send audio to, allowing
 ...
 [icon](@ref ABSenderPort::icon) property for the port. These
...
@page Sending-Receiving Sending and Receiving
 ...
 will describe how to use Audiobus input and sender ports directly.

 Both input and sender ports have very simple interfaces that are written in C and optimised for use
 ...
 store a pointer to the input or sender port object from a scope you can access directly, without
...
@section Audiobus-Sender Sending: The Audiobus Sender Port

 The Audiobus sender port class @link ABSenderPort @endlink provides a simple interface for
 transmitting audio in `AudioBufferLists`.

 To use the sender port, first specify the audio format (`AudioStreamBasicDescription`) that your app
 uses, by setting the port's [clientFormat](@ref ABSenderPort::clientFormat) property. Once
 set, the port will automatically perform conversion to the Audiobus line format.

 Then, simply call the [ABSenderPortSendAudio](@ref ABSenderPort::ABSenderPortSendAudio) C
...
@section Audiobus-Receiver Receiving: The Audiobus Receiver Port

 The Audiobus receiver port class @link ABReceiverPort @endlink provides an interface for receiving audio,
 either as separate audio streams (one per connected sender), or as a single audio stream with all
 sources mixed together.

 To begin using the class, as with the sender port, you must specify the audio format
 (`AudioStreamBasicDescription`) that your app uses, by setting the
 [clientFormat](@ref ABReceiverPort::clientFormat) property. Once set, the receiver port will
...
@subsection Lossless-vs-Low-Latency Lossless vs Low-Latency Audio
 ...
 The receiver port provides two separate versions of the audio stream: One containing lossless
 ...
 <img src="abinputport.png" title="Receiver port block diagram, showing lossless vs low-latency streams" />
 ...
 you record the raw audio feed as it comes directly from the receiver port, and use the live buffer for
 ...
 the lossless stream format, accessible via [ABReceiverPortReceive](@ref ABReceiverPort::ABReceiverPortReceive),
 ...
 ([ABReceiverPortReceiveLive](@ref ABReceiverPort::ABReceiverPortReceiveLive)). If you do, you will record
 ...
 1. First, as normal, you receive the Audiobus audio from the receiver port. **Note: You must process all the audio
...
@subsection Receive-Lossless Receiving Lossless Audio
 ...
 If you provide an [audio input block](@ref ABReceiverPort::audioInputBlock) to this class,
 then it will be called whenever an Audiobus audio packet is received.  Then, you can call
 [ABReceiverPortReceive](@ref ABReceiverPort::ABReceiverPortReceive) in order to access the
 received audio. See the [receiver port example recipe](@ref Input-Port-Recipe) above for an
 example.

 Alternately, you can let this class perform the buffering for you, and simply call
 [ABReceiverPortReceive](@ref ABReceiverPort::ABReceiverPortReceive) when you need audio.
 
 If you are using Audio Queues, you may wish to use [ABReceiverPortReceiveAQ](@ref ABReceiverPort::ABReceiverPortReceiveAQ)
 ...
 At any point, you can also call [ABReceiverPortPeek](@ref ABReceiverPort::ABReceiverPortPeek)
 to determine how much buffered audio is available and to determine the timestamp of the next
 buffered audio frames.

 You can also call [ABReceiverPortIsConnected](@ref ABReceiverPort::ABReceiverPortIsConnected)
...
@subsection Receive-Low-Latency Receiving Low-Latency Audio
 ...
 of the low-latency receive function @link ABReceiverPort::ABReceiverPortReceiveLive
 ABReceiverPortReceiveLive @endlink.
...
@subsubsection Receive-Streams-Sources Enumerating Current Sources
 ...
 [sources](@ref ABReceiverPort::sources) property, which returns an array of @link ABPort @endlink. 
...
@subsubsection Receive-Streams-Core-Audio-Callback Receiving From Within a Core Audio Callback
 ...
 To do so, you call ABReceiverPortPeek and ABReceiverPortReceive passing the source (@link ABPort @endlink) 
 you are interested in as the second argument.
 
 However, certain care must be taken. Please see the documentation for [ABReceiverPortReceive](@ref ABReceiverPort::ABReceiverPortReceive)
 and [ABReceiverPortEndReceiveTimeInterval](@ref ABReceiverPort::ABReceiverPortEndReceiveTimeInterval) for more info.
...
@subsubsection Receive-Streams-Monitoring Providing Live Monitoring when Receiving and Manipulating Separate Streams
 ...
 1. First, as normal, you receive each audio stream from the receiver port. **Note: You must process all the audio
    that is present, and you must not expect any particular quantity of audio to be available.** In the event of
    stalls, you may receive 0 frames at a given time interval, then an increased number of frames later, while
    the system is catching up. If you only ever process a set amount of audio, then your app will never catch up,
    and will experience greater and greater latency.
...
@page Filtering Filtering
 ...
 You may wish to offer users more flexible operation by also offering an receiver port, which will allow
 users to specify your app as an output in the Audiobus connection graph.

 You can create both a filter port, as described above, as well as an [receiver port](@ref Audiobus-Receiver),
 ...
 [low-latency input methods](@ref Receive-Low-Latency) of the Audiobus receiver port.
...
@section Senders-Timestamps Senders, Send Audio Timestamps
 ...
 care of for you. Otherwise, if you are using @link ABSenderPort @endlink directly, make sure you are
...
@section Receivers-Timestamps Receivers, Use Audio Timestamps
 ...
 Note that if you're using the Audio Unit Wrapper with the receiver port and you receive Core Audio input
 ...
 If you are using the Audio Unit Wrapper with the receiver port but receiving audio via the audio unit's 
 render callback instead (`AUGraphSetNodeInputCallback` or `kAudioUnitProperty_SetRenderCallback`), 
 you will need to use [ABReceiverPortPeek](@ref ABReceiverPort::ABReceiverPortPeek) to retrieve the timestamp.
...
@section Designated-Output Designated Output
 ...
 then out the Audiobus speaker sender port, then you don't want the synth app playing the same
 audio that will be coming out of the speaker.

 So, Audiobus defines [port attributes](@ref ABReceiverPort::attributes) which allow your app to
 ...
 @code
 input.attributes = ABReceiverPortAttributePlaysLiveAudio;
 @endcode
 ...
 The Audiobus sender ports provide you with the attributes for all currently connected receivers via
 the [connectedPortAttributes](@ref ABSenderPort::connectedPortAttributes) property. It's your responsibility
 ...
 [ABSenderPortGetConnectedPortAttributes](@ref ABSenderPort::ABSenderPortGetConnectedPortAttributes), which
 can safely be used in a Core Audio context, unlike the Objective-C equivalent.
 
 if:
 
 @code
 (theSenderPort.connectedPortAttributes & ABReceiverPortAttributePlaysLiveAudio)
 @endcode
 
 or, in C only,
 
 @code
 (ABSenderPortGetConnectedPortAttributes(theSenderPort) & ABReceiverPortAttributePlaysLiveAudio)
 @endcode
 
 that is, if the `ABReceiverPortAttributePlaysLiveAudio` flag is present - then you need to mute your app's
 output.
 
 See the [sample code](@ref Sender-Port-Recipe) for an example implementation.
